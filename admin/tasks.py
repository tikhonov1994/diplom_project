...
def notify_new_popular_reviews(mailing_id: str, params: dict):
    mailing = Mailing.objects.get(id=UUID(mailing_id))
    with requests.get(
            'social_api:8010/social_api/api/v1/reviews/daily-top-reviews') as reviews_response:
        ...
        for i in range(len(reviews)):
            with requests.get(f'api:8001/content/api/v1/films/{reviews[i]["film_id"]}') as film_response:
