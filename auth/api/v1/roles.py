...
@router.get('', response_model=list[UserRoleSchema],
            ...
            operation_id="authorize_get_roles",
            tags=['authorize']
            )
...
@router.post('', description='Добавить пользовательскую роль',
            operation_id="authorize_add_role",
            tags=['authorize']
             )
...
@router.delete('/{role_id}', description='Удалить пользовательскую роль', operation_id="authorize_delete_roles", tags=['authorize'])
