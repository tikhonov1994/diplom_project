...
async def test_delete_not_existing_role(http_auth_client: ClientSession) -> None:
    async with http_auth_client.delete(ENDPOINT + str(uuid.uuid4()), headers=test_auth_headers) as response:
...
async def test_delete_role_conflict(http_auth_client: ClientSession, db_session: AsyncSession) -> None:
    ...
    async with http_auth_client.delete(ENDPOINT + test_user_info['user_role_id'], headers=test_auth_headers) as response:
        assert response.status == HTTPStatus.CONFLICT
