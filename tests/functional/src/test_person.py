...
async def test_get_by_id(http_client) -> None:
    async with http_client.get(f'/api/v1/persons/{test_persons[0]["id"]}') as response:
...
async def test_get_invalid_id(http_client) -> None:
    async with http_client.get(f'/api/v1/persons/{uuid.uuid4().hex}') as response:
        assert response.status == HTTPStatus.NOT_FOUND


async def test_get_films_by_person_id(http_client) -> None:
    async with http_client.get(f'/api/v1/persons/{test_persons[0]["id"]}/film') as response:
        data = await response.json()
        assert response.status == HTTPStatus.OK
        assert len(data) == 2
...
async def test_get_by_id_from_cache(http_client, add_data_to_index, delete_data_from_index) -> None:
    ...
    async with http_client.get(f'/api/v1/persons/{test_person.get("id")}') as new_response:
